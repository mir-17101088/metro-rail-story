...
- The route game is a separate chunk, stylesheet included. Its code is downloaded at the first idle moment after the story is readable, but the game is only built (two lists of 80 stations, its section laid out) once the reader heads for it: the timeline coming within a screen, a masthead jump, or a link to `#game`. Built at that idle moment, it ran while the story map was still drawing its opening tiles, for every reader, including those who never scroll that far.
- The map starts once the loading screen is actually on screen (the browser's `first-contentful-paint` entry, `afterFirstPaint` in `src/main.ts`; one second at most). Started straight away, its WebGL test context and mapbox's shaders kept the GPU busy and the loading screen's first frame waited behind them, so a phone showed a blank page first. The scroll-step observer also waits a frame before measuring the screen (`src/scroll/steps.ts`), which otherwise forced the page's whole first layout inside the start-up script.
- Timeouts on map work count only the time the tab is visible (`src/lib/visible-time.ts`): the map style's 20 seconds on both maps and the loading screen's 60-second ceiling. Mapbox applies a style on an animation frame and a background tab gets none, so a story opened in a background tab (middle-click, "open in new tab") used to give up on its map before anyone looked and show the "could not load" fallback.
...
- A back-to-top button appears once the reader has scrolled past the map. It steps aside while the route game (its trip panel or its map) fills the bottom of the screen, where it would cover the map's attribution or, on a phone, the times and fares of the route options.
- Phones held sideways (short, wide screens): the masthead is 3.25rem instead of 4.25rem, the explorer sheet opens at about a third of the screen, and the page gutters never fall inside a notch (`--gutter` takes the safe-area insets; the maps' corner controls move in from them too).
...
- **What the reader sees:** the same station twice gives "You can just walk"; one route plays at once; several are listed (hover or focus previews one on the map). On wide screens a waiting choice is not tucked under the station fields, where readers missed it: it opens in a card in the middle of the map, headed "Choose a route", with the map dimmed around it, and the panel points to it. Once a route is picked the options return under the fields, clear of the ride, so another can still be tried (`Game.placeOptions`). Phones keep them under the fields, directly above the map; no connection gives "No metro route exists" (every pair of stations is connected today, so this only guards against future data changes). Reset, or "Plan another trip" on the result card, starts over; "Plan another trip" also scrolls back to the "Plan your trip" panel, which on a phone sits above the map, off screen after a ride. The card's close button (or Escape) puts it away and frames the trip on the whole map.
- **On phones** (`src/game/picker.ts`, `Game.showOutcome`, the phone sections of `game.css`):
  - Tapping a station field lifts it to just under the masthead before the keyboard arrives, and the list is sized to the room left between the field and the keyboard (the visual viewport), so it is never opened behind the keyboard. The list scrolls itself, never the page.
  - Once both stations are in, the keyboard closes and the page brings up what comes next: the choice of routes under the fields, or the map with the answer.
  - Route options are laid out in two rows at every width, desktop included (lines and numbers, then the name and details), so badges, times and names line up down the list. On narrow screens (under 480px) the ride's caption does the same (badges and "Skip ride", then the words). Below 346px the fields use short placeholders ("Starting station", "Destination").
  - Held sideways (landscape, under 541px tall), the panel sits to the left of the map instead of above it, and the map stays pinned beside it while the reader scrolls through the route options. The map is never taller than the screen on short screens.
...
- Canonical URL, description, robots directives, Open Graph, X/Twitter card and JSON-LD (`NewsArticle`, `WebPage`, `NewsMediaOrganization`, contributors) are written directly in `index.html`. **Update `datePublished` / `article:published_time` (currently 2026-09-21) if the story goes live on another date.**
...
- **Cost section copy** (as supplied, en dash replaced with a comma): now reads "is forecast to be 1,339,197 daily boardings" (the desk's correction of 2026-09-19).
