...
- `index.html` paints a loading screen on the first frame (inline CSS, no script needed). It draws the real network while the map loads, and lifts once the map has painted its opening view and the fonts are in. It waits at most 5 seconds on the map and never stays longer than 9 seconds in total; if the map is slower, the story shows and the map fades in when ready (and after 8 seconds the stage stops hiding a half-drawn map rather than showing nothing). A CSS-only failsafe hides the screen after 20 seconds even if scripts never run.
...
- The map's first frame is `load` (style up, opening tiles drawn), not `idle` (every tile settled): `StoryMap.painted` vs `StoryMap.ready`. Waiting for the latter used to leave a reader on an empty stage when the loading screen's cap ran out first.
